...
int ipv4_route_lookup ( ipv4_route_t * route, ipv4_addr_t addr ){
	...
	memcpy(mascara, route->subnet_mask, IPv4_ADDR_SIZE);
	memcpy(subnet, route->subnet_addr, IPv4_ADDR_SIZE);
 ...
 *
 * DESCRIPCIÓN:
 *   Esta función leer el fichero especificado y añade las rutas IPv4 leidas
...
int ipv4_route_table_read ( char * filename, ipv4_route_table_t * table )
{
  ...
  while ((! feof(routes_file)) && (err==0)) {
    ...
    err = sscanf(line, "%s %s %s %s\n", 
                 subnet_str, mask_str, iface_name, nhop_str);
    if (err != 4) {
      fprintf(stderr, "%s:%d: Invalid IPv4 Route format: \"%s\" (%d items)\n",
              filename, linenum, line, err);
      fprintf(stderr, 
              "%s:%d: Format must be: <subnet> <mask> <iface> <next hop>\n",
              filename, linenum);
      err = -1;
      ...
      ipv4_route_t * new_route = 
        ipv4_route_create(subnet, mask, iface_name, next_hop);
 ...
 *   la salida indicada.
 */
int ipv4_route_table_output ( FILE * out, ipv4_route_table_t * table )
{
  int err = fprintf
...
int ipv4_route_table_write ( ipv4_route_table_t * table, char * filename )
{
  ...
  if (table != NULL) {
    num_routes = ipv4_route_table_output (routes_file, table);
    if (num_routes == -1) {
...
{
  if (table != NULL) {
    ipv4_route_table_output (stdout, table);
  }
}
